...
function _tabel(v){
    ...
    v.data.forEach((v1,i1) => {
        ...
        if(faction){
            fdata+=`<td style="min-width: 15%;justify-content: center;display: flex;">`+_btnTabel(v.action,v.no-2)+`</td>`;
        }
        fdata+=`</tr>`;
...
function btabelForExcel(v){
    ...
    v.data.forEach((v1,i1) => {
        ...
        if(faction){
            fdata+=`<td>`+_btnTabel(v.action,v.no-2)+`</td>`;
        }
        fdata+=`</tr>`;
    });
